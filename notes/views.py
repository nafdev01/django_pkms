...
def course_detail(request, id, slug):
    ...
    context = {"student": student,"course": course, "topics" : topics,
    }
    return render(request, template_path, context)
...
def topic_detail(request, id, slug):
    ...
    other_topics = Topic.objects.filter(course_id=topic.course.id, course__student_id=student.id).exclude(id=topic.id)
    subtopics = topic.subtopic_set.filter(topic__course__student_id=student.id)
    
    template_path = "notes/detail/topic_detail.html"
    context = {"student": student,"topic": topic, "subtopics" : subtopics, "other_topics": other_topics
    }
    return render(request, template_path, context)
...
def subtopic_detail(request, id, slug):
    student = request.user
    subtopic = get_object_or_404(SubTopic, id=id, slug=slug, topic__course__student_id=student.id)
    other_subtopics = SubTopic.objects.filter(topic_id=subtopic.topic.id, topic__course__student_id=student.id).exclude(id=subtopic.id)
    ...
    context = {"student": student,"subtopic": subtopic, "other_subtopics": other_subtopics, "entries" : entries,
    }
    return render(request, template_path, context)
...
def entry_detail(request, id, slug):
    student = request.user
    entry = get_object_or_404(Entry, id=id, slug=slug, subtopic__topic__course__student_id=student.id)
    other_entries = Entry.objects.filter(subtopic_id=entry.subtopic.id, subtopic__topic__course__student_id=student.id).exclude(id=entry.id)
        ...
        if form.is_valid():
            try:
                ...
                if 'duplicate key value violates unique constraint' in str(e):
                    messages.error(request, f'A topic with that number already exists in this course "{course.course_code}".')
                else:
    ...
    if request.method != "POST":
        ...
        if form.is_valid():
            try:
                ...
                if 'duplicate key value violates unique constraint' in str(e):
                    messages.error(request, f'A subtopic with that number already exists in this topic "{topic}".')
                else:
...
def create_entry(request, subtopic_id):
    student = request.user
    subtopic = get_object_or_404(SubTopic, id=subtopic_id, topic__course__student_id=student.id)
    if request.method != "POST":
        form = EntryForm()
    else:
        ...
        if form.is_valid():
            try:
                ...
                if 'duplicate key value violates unique constraint' in str(e):
                    messages.error(request, f'An entry with that title already exists in subtopic "{subtopic}".')
                else:
...
def delete_subtopic(request, subtopic_id):
    student=request.user
    subtopic = SubTopic.objects.get(id=subtopic_id, topic__course__student_id=student.id)
    subtopic_name = subtopic.name
    topic = subtopic.topic
    
    subtopic.delete()
    messages.success(request, f"The subtopic {subtopic_name} has been deleted successfully.")
...
def delete_entry(request, entry_id):
    student=request.user
    entry = Entry.objects.get(id=entry_id, subtopic__topic__course__student_id=student.id)
...
def search(request):
    ...
    if query:
        objectives = Objective.objects.filter(Q(name__icontains=query) | Q(description__icontains=query), topic__course__student_id=student.id)
        terms = Term.objects.filter(Q(name__icontains=query) | Q(definition__icontains=query), course__student_id=student.id)
        entries = Entry.objects.filter(Q(name__icontains=query) | Q(content__icontains=query), subtopic__topic__course__student_id=student.id)
        subtopics = SubTopic.objects.filter(Q(name__icontains=query), topic__course__student_id=student.id)
        topics = Topic.objects.filter(Q(name__icontains=query) | Q(overview__icontains=query), course__student_id=student.id)
        courses = Course.objects.filter(Q(name__icontains=query) | Q(course_type__icontains=query) | Q(course_code__icontains=query) | Q(about__icontains=query) , student_id=student.id)
    else:
        objectives = Objective.objects.filter(topic__course__student_id=student.id)
    ...
    context = {"terms": terms, "entries": entries, "subtopics": subtopics, "topics": topics, "courses": courses , "objectives": objectives }
    return render(request, template_path, context)
...
def entry_share(request, entry_id):
    ...
    entry = get_object_or_404(Entry, id=entry_id, subtopic__topic__course__student_id=student.id)
    ...
    message = get_template("notes/includes/entry_email_template.html").render({'entry': entry, "entry_url": entry_url})
    mail = EmailMessage(
        subject=subject,
        body=message,
        from_email=sender,
    ...
    if mail.send():
        messages.success(request, f"The entry '{entry}'was successfully shared with '{recipient}'")
    else:
        messages.error(request, f"The entry '{entry}' could not be shared with '{recipient}'")
